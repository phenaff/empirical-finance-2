...
```{r, label=discmodbinomial2-setup, echo=FALSE, results='hide'}
  library(fOptions)
...
We would like to verify the convergence of the Cox-Ross model to the
...
```{r, binomial-plot, fig=F, echo=T, eval=FALSE}
  par(mfrow=c(1, 2))
...
```{r, binomial-plot2, fig=F, echo=F, eval=FALSE}
  par(mfrow=c(1, 2))
  plot(NN, res[, 1], type='l',main='ATM', xlab='Number of steps', ylab='price', col='blue', ylim=c(min(res[,1]), max(res[,1])))
...
```{r, label=CRRWithDrift-plot-calc, echo=F}
...
```{r, CRRWithDrift-plot, fig=F, echo=F}
  par(mfrow=c(1, 2))
