...
```{r, init-FIRiskManagement,echo=TRUE,results='hide'}
  library(linprog)
  library(xtable)
  library(empfin)
```

In this chapter, we introduce the two broad categories of Fixed Income
...
$$\begin{aligned}
  ...
  &=& V_T \left[ T \frac{\sum_i F_i (1+r)^{-T_i}}{V_T} - \frac{\sum_i F_i T_i (1+r)^{T_i}}{V_T} \right](1+r)^{T-1} \end{aligned}$$

In the last expression, we recognize the definition of Duration
(\[eq:macaulay\]), so that the Variation of $V_T$ is finally:
...
$r=.05$. The investment portfolio will be made of two bonds described in
Table \[tab:bonds-ab\]
...
$$\begin{aligned}
V_0 & = & q_A P_A + q_B P_B \\
T & = & q_A \frac{P_A D_A}{V_0} + q_B \frac{P_B D_B}{V_0}\end{aligned}$$
...
```{r, label=fig-immunization, echo=FALSE, fig=TRUE}
  cl <- c('red', 'green')
...
Figure \[fig:immunization\] plots the value of the investment portfolio
...
$L(t), t=1, \ldots, 5$ summarized in Table \[tab:liability\]. In order
to meet this liability, you can invest in a portfolio of 5 bonds
described in table \[tab:bonds-2\].
...
\begin{align}
    \mbox{min} & \sum_i q_i P_i \label{eq:objective} \\
    \mbox{s.t.} & \nonumber \\
...
The optimal portfolio composition is shown in Table \[tab:sol1\].

```{r, label=fi-rm-7, results='asis',echo=F,fig=F}
  z <- as.matrix(lp.sol$solution[1:5])
  colnames(z) <- c('Quantity')
  xres<-xtable(z, digits=2)
```

```{r, label=fi-rm-8, results='asis',echo=F,fig=F}
...
The optimal portfolio is displayed in table \[tab:sol3\]. We no longer
have a perfect match, and some cash must be reinvested from period to
period, as shown in table \[tab:sol2\]: a cash balance is re-invested
from period 2 to period 3.

```{r, label=fi-rm-11, results='asis',echo=F,fig=F}
  z <- as.matrix(cbind((1:5), res2$solution[5:9]))
  colnames(z) <- c('Period', 'Cash Balance')
  xres<-xtable(z, digits=2)
```

```{r, label=fi-rm-12, results='asis', echo=F, fig=F}
  print.xb(xres, include.colnames=T)
```

```{r, label=dedication-sol-2, results='asis',echo=F,fig=F}
  z <- as.matrix(res2$solution[1:4])
  colnames(z) <- c('Quantity')
  xres<-xtable(z, digits=2)
```

```{r, label=fi-rm-13, results='asis', echo=F, fig=F}
  print.xb(xres, include.colnames=T)
```

```{r, label=fi-rm-14, echo=TRUE}
  print(paste('dedication cost:', round(res2$opt,2), 'PV of liability:', round(PV,2)))
```

The dedication cost is naturally higher than the present value of the
liability, since the solution involves reinvesting cash from the second
