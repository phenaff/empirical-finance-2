...
In this section, we investigate strategies for mitigating two risks:

1.  the volatility is unknown

2.  the implied volatility is a function of strike and time to maturity

### The nature of volatility risk

See the part “Stylized Facts” for a description of the volatility smile.

### Sticky strike and sticky delta

### Model-free hedging of smile risk

Butterfly

:   $\frac{\sigma_+ - \sigma_-}{\sigma_0}$
...
$$\begin{aligned}
    \sigma_+ &=& \sigma_0 + BF + \frac{1}{2} RR \\
    \sigma_- &=& \sigma_0 + BF - \frac{1}{2} RR \\\end{aligned}$$

Example:

1.  Assume a quadratic smile:
...
3.  when the gamma changes sign, the Black-Scholes model is no longer an
    ...
    be shoothed with a partial static hedge before using the
